...
interface DialerProps {
  ...
  onCall?: () => void; // NEW: Callback for call action
}

const Dialer: React.FC<DialerProps> = ({ 
  isCollapsed = false, 
  initialNumber, 
  onCall // NEW: Use callback instead of router
}) => {
  ...
  // UPDATED: Handle call button click with state management
  const handleCall = (): void => {
    if (phoneNumber.length === 10) {
      ...
      setTimeout(() => {
        ...
        if (callSuccess) {
          // CHANGED: Use callback instead of router.push
          if (onCall) {
            onCall();
          }
        } else {
